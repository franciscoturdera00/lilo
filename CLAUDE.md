...
- `cad-workshop` — native stdio MCP served from the sibling project's own uv venv (`cadw-mcp`): `check` / `ship` / `slice_stl` / `doctor` for 3D-printable parts. `ship` slices without the Orca inspection pause — only call it after the operator has approved the design. Tool results carry `data.files_written` for Telegram attachment. New design requests ("make me a 3d model of X", photo attached or not) go to the cad-workshop PM via its `.lilo-inbox/` — these MCP tools only operate on parts/files that already exist. The PM searches Thingiverse/Printables for an existing design first and will come back with candidates and a route choice for the operator.
...
Both burn subagent context on each cron tick, not Lilo's.
